...
- `components/` — skeleton, Suspense loader, inline load-error recovery
- Public API: `index.ts` exports only `{ TransactionsView }` (locked by `index.test.ts`)

**API layer** (`src/lib/api/`):

- `transactions.contract.ts` — shared types + `TransactionsApi` interface
- `transactions.schema.ts` — Zod schemas + `parse*` helpers (single wire-format gate)
- `api-error.ts` — `ApiError`, `runRoute`, `toUserMessage`, structured `{ code, message }` bodies
- `transactions.mock.ts` — in-memory impl (`satisfies TransactionsApi`); seed validated via `parseTransactionsList`
- `transactions.client.ts` — `fetch` wrappers; failed responses → `apiErrorFromResponse`, success JSON → schema parsers
- `transactions.routes.ts` — single source of truth for paths (client + MSW)
- `transactions.ts` — entry re-export; server loader uses mock `listTransactions` directly today
- Route handlers under `src/app/api/transactions/` delegate to the mock and wrap handlers in `runRoute`

**Wire-format validation:** Every boundary that crosses the HTTP seam parses through `transactions.schema.ts`. Invalid params, malformed JSON, or wrong shapes become `ApiError` with codes `INVALID_INPUT` or `VALIDATION` — never raw `ZodError` or unchecked casts. The mock and client both `satisfies TransactionsApi` so compile-time and runtime contracts stay aligned.

**Error handling:** Route handlers never leak stack traces. `runRoute` maps thrown `ApiError` to JSON + status; anything else becomes `500 UNEXPECTED` with a generic message. The client rehydrates `ApiError` from failed responses. UI surfaces errors via `toUserMessage()` (safe copy only) — inline `TransactionsLoadError` on initial load (`router.refresh()` retry), toasts for invoice download and bulk-retry catastrophes, plus `error.tsx` / `global-error.tsx` with `logAppError` for boundary failures.

**RSC boundaries:** `page.tsx` paints the shell immediately; `TransactionsLoader` fetches inside `<Suspense>` with `TransactionsViewSkeleton` as fallback. Load failures return an inline error card instead of crashing the segment.
...
| Schema + client          | Zod parse helpers, malformed JSON, structured API error responses |
| API errors               | `ApiError` serialization, `toUserMessage`, `runRoute`             |
