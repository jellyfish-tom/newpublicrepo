...
- **API surface split.** `transactions.contract.ts` (types), `transactions.mock.ts` (in-memory implementation), `transactions.client.ts` (fetch wrappers), `transactions.ts` (entry that re-exports the chosen impl). Route handlers under `src/app/api/...` delegate to the mock.
